...
* **Fix (Action):**
  * Apply `select_related()` for single-valued relationships (SQL JOIN).
  * Apply `prefetch_related()` for many-to-many/reverse relationships.
  * Use `defer()` / `only()` to defer loading unneeded heavy fields (e.g., raw JSON or text blobs).
* **Result:** Reduced database queries from N+1 (e.g., 501 SQL queries) to 2 queries, dropping latency from 4,000ms down to ~150ms.

---

### 2. AWS Microservices & Container Failures

**Question:** *"An ECS task running on Fargate keeps crashing shortly after starting and enters a crash / restart loop. How do you troubleshoot this?"*
...
* **Action:**
  ...
  2. **Rate-Limiting Workers:** Configure Celery's `rate_limit` or implement token-bucket throttling in Redis.
...
* **Action (Multi-Phase Deployment Pattern):** Never rename/delete columns directly in a single release. Use a 3-step roll-out:
  ...
  2. **Phase 2 (Backfill Data):** Run a background script using `bulk_update()` / `iterator()` to copy existing data from the old column to the new column.
...
* **Diagnose:** Use memory profilers like `tracemalloc` or `memory_profiler` to trace object allocation in Python.
* **Common Root Causes & Fixes:**
  * *ORM QuerySet Caching:* Django caches all database queries when `DEBUG=True`. Ensure `DEBUG=False` in production.
  * *Unbounded Data Loading:* Iterating over huge QuerySets (`list(qs)`) loads all model objects into memory at once. Fix by using `.iterator()`.
  * *Celery Workers:* Set worker recycling limits in Celery using `worker_max_tasks_per_child` so worker processes restart clean after executing a batch of tasks.
...
* **Short-Term (Emergency):**
  ...
  2. Introduce or increase **Redis / ElastiCache** caching for heavily read API endpoints using `@cache_page` or low-level `cache.set()`.
...
* **Authentication/Authorization:** Implement JWT authentication (e.g., `djangorestframework-simplejwt`) with short-lived access tokens and refresh tokens. Use DRF `permission_classes` to enforce Role-Based Access Control (RBAC).
* **Secret Management:**
  ...
  * Enforce IAM Roles for Tasks (`taskRoleArn`) so containers access AWS resources without static AWS access keys.
...
* **Mixing Async & Sync safely:** In Python/Django, running blocking synchronous ORM calls inside an `asyncio` loop blocks the entire event loop. Wrap sync calls using `sync_to_async()` or use Django's native async ORM methods (`aget()`, `acreate()`).
...
* **Immediate Containment:**
  1. Trigger an immediate rollback on **AWS ECS** to re-point the Application Load Balancer target group to the previous stable Docker image tag / task definition.
  2. Revert the commit in Git.
* **Root Cause Analysis (RCA) & Prevention:**
  1. Add automated CI pipeline checks (GitHub Actions / GitLab CI): enforce unit tests (`pytest`), integration tests, and static linting (`ruff`, `mypy`, `bandit`) before permitting PR merges.
...
* **Action Plan:**
  1. **Establish a Baseline:** Set up static analysis tools (`ruff`, `mypy`) and error tracking (`Sentry`) to map active runtime errors and code complexity without altering code logic.
  2. **Add Test Coverage:** Write end-to-end integration tests (`pytest`) around critical core business flows (e.g., payment processing) to create a safety net before touching existing code.
...
Technical screen for a **Python + Django + AWS** role organized jointly with **Tata Consultancy Services (TCS)** and **Cisco** interviewers (`@cisco.com` and `@tcs.com` attendees present).

---

### 1. The Elevator Intro (30–60 Seconds)

When they ask *"Tell us about yourself,"* anchor on your senior backend/cloud core while highlighting scale:

> *"I'm a Senior Software Engineer with over 15 years of experience building scalable backend microservices, REST APIs, and cloud infrastructure. My primary technical stack centers around Python, Django, FastAPI, and AWS. At Fidelity I built Django REST Framework APIs for enterprise network automation; at Disney I worked on financial backends processing $10M+ daily on AWS. Most recently through CGJSoftware I've architected containerized microservices on ECS/Fargate. I'm looking to bring that full-stack backend ownership into a dedicated enterprise team at Cisco."*
...
* **Django Request-Response Lifecycle:** Middleware → URL Routing → View Execution → ORM/Templates/DRF Serializer → Response Middleware.
...
4. *"How is day-to-day collaboration structured between TCS and Cisco engineers on this team?"*

---

### 5. Resume Story Map (STAR Anchors)

| Topic | Your story | Key metric |
|-------|-----------|------------|
| Django / DRF | Fidelity — network device automation API | 4 hrs → 10 min provisioning |
| AWS scale | Disney — financial reporting backend | $10M+ daily, 99.99% uptime |
| Microservices | CGJSoftware — ECS/Fargate Python services | &lt;120ms latency, +45% throughput |
| Legacy modernization | CVS — monolith decoupling | +50% integration test speed |
| Security / HIPAA | Alan Health — REST APIs | 500k user profiles |
| Cisco rapport | Hospira — Cisco ACS wireless security testing | 5 formal verification run-throughs |

---

### 💡 Quick Interview Logistics Checklist

* **Platform:** Cisco Webex (download/update 15 minutes prior).
* **Schedule:** Confirm exact date/time in your calendar invite (Thursday or Friday).
* **Setup:** Test microphone, camera, and lighting. Have resume PDF open, pen and paper for diagrams.
* **Coding:** If live coding, restate problem, discuss edge cases, talk through approach before typing.
